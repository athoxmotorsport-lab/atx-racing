(() => {
  ...
  const renderProfile = payload => {
    ...
    profile.querySelector('[data-profile-rank]').textContent = rating?.performance_class?.toUpperCase() || 'ROOKIE';
    ...
    const performance = profile.querySelector('[data-performance-badge]');
    const performanceTier = rating?.performance_class || 'rookie';
    performance.dataset.tier = performanceTier;
    performance.dataset.fr = `Performance · ${performanceTier}`;
    performance.dataset.en = `Performance · ${performanceTier}`;
    const safe = profile.querySelector('[data-safe-badge]');
    const safeTier = rating?.safety_class || 'bronze';
    safe.dataset.tier = safeTier;
    safe.dataset.fr = `SAFE · ${safeTier}`;
    safe.dataset.en = `SAFE · ${safeTier}`;
