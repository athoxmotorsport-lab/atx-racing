...
for(const path of ["course.html","profil-pilote.html"]){const html=await readFile(join(root,path),"utf8");const canonical=html.match(/<link rel="canonical" href="([^"]+)">/);assert(canonical,"Missing canonical on "+path);for(const language of ["fr","en"])assert(html.includes('<link rel="alternate" hreflang="'+language+'" href="'+canonical[1]+'">'),"Missing "+language+" hreflang for "+path);}
