(() => {
  ...
  const editWrap = section.querySelector('[data-admin-edit-wrap]');
  const editForm = section.querySelector('[data-admin-edit-form]');
  let events = [];
  ...
  const showEditor = event => {
    const schedule = scheduleByKey(event);
    const p1 = schedule.get('practice_1') || {};
    const briefing = schedule.get('briefing') || schedule.get('practice_2') || {};
    const qual = schedule.get('qualifying') || {};
    const race = schedule.get('race') || {};
    editForm.dataset.slug = event.slug;
    editForm.innerHTML = `
      ...
      ${select('Format','eventType',event.event_type,[['daily_race','Daily Race'],['sprint','Sprint'],['endurance','Endurance'],['championship','Championship'],['special_event','Special Event']])}
      ${field('Durée de course (minutes)','durationMinutes',event.duration_minutes,'number','min="1" max="1440" required')}
      ${field('Pilotes maximum','maxDrivers',event.max_drivers,'number','min="1" max="100" required')}
      ...
      <div class="form-field full"><button class="btn primary" type="submit">Enregistrer les modifications</button></div>`;
  ...
  const render = () => {
    ...
    events.forEach(event => {
      ...
      card.className = 'admin-event-row';
      const local = new Intl.DateTimeFormat('fr-BE',{dateStyle:'medium',timeStyle:'short',timeZone:'Europe/Brussels'}).format(new Date(event.starts_at));
      card.innerHTML = `<div><strong>${esc(event.title_fr || event.circuit_name)}</strong><small>${esc(local)} · ${esc(event.circuit_name)}${event.is_public ? '' : ' · RETIRÉ'}</small></div><div class="admin-event-actions"><button class="btn small" type="button" data-edit>Modifier</button>${event.is_public ? '<button class="btn small danger" type="button" data-remove>Retirer</button>' : ''}</div>`;
      card.querySelector('[data-edit]').addEventListener('click', () => showEditor(event));
      card.querySelector('[data-remove]')?.addEventListener('click', async () => {
        if (!confirm(`Retirer « ${event.title_fr || event.circuit_name} » du site ?`)) return;
        try {
          await request('DELETE',{slug:event.slug});
          status.textContent = 'Événement retiré du site.';
          await load();
        } catch { status.textContent = 'Impossible de retirer cet événement.'; }
      });
      list.append(card);
    });
    if (!events.length) list.innerHTML = '<p class="empty-state">Aucun événement enregistré.</p>';
  };
  ...
  const readImage = file => new Promise((resolve,reject) => {
    if (!(file instanceof File) || !file.size) return resolve({imageData:null,imageType:null});
    if (file.size > 5 * 1024 * 1024) return reject(new Error('image_too_large'));
    const reader = new FileReader();
    reader.onload = () => resolve({imageData:String(reader.result).split(',')[1],imageType:file.type});
